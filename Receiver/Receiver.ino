...
int py;
Servo servo;
RF24 radio(7,8); // selecting the Chip enable and chip select not pin on the second arduino UNO 
...
void loop(){
if(radio.available()) {
  radio.read(&receivedData, sizeof(receivedData)); // when the signal is detected, the data will be read by the receiver module 
...
if(yAxis < value1 || yAxis > value2 )
{
  if(yAxis < value1){
    ...
    // the motor will turn all the wheels backwards when the joystick is pulled downwards 
...
if(xAxis < value1 || xAxis > value2){
  servo.write(x);
  //Serial.println(x);                  // sets the servo position according to the scaled value
  delay(15);     
  }
else {
